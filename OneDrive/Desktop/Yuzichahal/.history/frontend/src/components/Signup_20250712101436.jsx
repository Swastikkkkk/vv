...
const Signup = () => {
  ...
  const [role, setRole] = useState(''); // initially empty to force selection
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      // Ask for role
      const confirmedRole = prompt('Enter your role: admin or user').toLowerCase();

      if (!confirmedRole || (confirmedRole !== 'admin' && confirmedRole !== 'user')) {
        setError('Invalid or no role provided. You must specify "admin" or "user".');
        return;
      }
      ...
      alert(`Signed up as ${confirmedRole}`);
      // Redirect based on role
      if (confirmedRole === 'admin') navigate('/admin-dashboard');
