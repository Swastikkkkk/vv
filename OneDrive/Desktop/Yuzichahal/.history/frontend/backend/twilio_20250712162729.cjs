...
const { TWILIO_SID, TWILIO_AUTH_TOKEN, TWILIO_PHONE, VAPI_API_KEY, ASSISTANT_ID } = process.env;
const VoiceResponse = twilio.twiml.VoiceResponse;
...
app.post("/api/call", async (req, res) => {
  ...
  try {
    ...
    const call = await client.calls.create({
      to,
      from: TWILIO_PHONE,
      url: `http://localhost:5800/twilio/voice`,  // Localhost works if testing locally with Twilio CLI or similar
      method: "POST",
    });
    res.json({ success: true, callSid: call.sid });
  } catch (e) {
